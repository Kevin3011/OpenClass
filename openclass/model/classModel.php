...
namespace Model{
    ...
    class classModel extends dbHelper{
        ...
        public function getClassListByCategory($category){
            try{
                $con = self::db();
                $stmt = $con->prepare("SELECT cl.class_code,cl.name AS classname, cl.filePath,cl.description,cl.type,cl.isActive,us.name FROM class AS cl 
        ...
        public function getClassListByUser($user){
            try{
                $con = self::db();
                $stmt = $con->prepare("SELECT cl.name AS classname,cl.filePath, cl.description,gr.dateJoin,cl.class_code FROM group_class_user AS gr 
                                                LEFT JOIN user AS us ON gr.user_id = us.id
                                                LEFT JOIN class AS cl ON gr.class_id = cl.id
        ...
        public function getClassListByOwner($user){
            try{
                $con = self::db();
                $stmt = $con->prepare("SELECT cl.name AS classname,cl.filePath,cl.description,cl.class_code, cl.createDate FROM class AS cl 
                                                LEFT JOIN user AS us ON cl.creator_user_id = us.id
                                                WHERE us.id_user = ?");
                $stmt->execute(array($user));
                $result = $stmt->fetchAll();
        ...
        public function getClassByCode($code){
            try{
                $con = self::db();
                $stmt = $con->prepare("SELECT cl.class_code,cl.name AS classname, cl.filePath,cl.description,cl.type,cl.isActive,us.name,us.id_user,cc.name AS category_name FROM class AS cl 
                                                    LEFT JOIN user AS us ON cl.creator_user_id = us.id
        ...
        public function addClass($name, $description, $filePath, $category, $type, $createDate, $id_user){
            try{
                $con = self::db();
                $query = "INSERT INTO class 
                            (class_code,name,description,filePath,type,createDate,isActive,creator_user_id,class_category_id)
                        VALUES 
                            (:class_code, :name, :description, :filePath, :type, :createDate, :isActive, (SELECT MAX(id) FROM user WHERE id_user = :id_user), (SELECT MAX(id) FROM class_category WHERE category_code = :category))";
